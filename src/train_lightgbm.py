...
# ── GPU config ────────────────────────────────────────────────────────────────
# RTX 5060 — LightGBM uses "cuda" device.
# Requires: pip install lightgbm --install-option=--gpu  OR the prebuilt GPU wheel.
...
def _make_clf(**kwargs):
    """Return an LGBMClassifier with GPU params merged in."""
    return LGBMClassifier(
        random_state=42,
        verbose=-1,           # suppress LightGBM's own per-tree logs
        **GPU_PARAMS,
        **kwargs,
    )
...
def find_best_n_estimators(X_train, X_test, y_train, y_test, le, estimator_range=range(10, 20)):
    ...
    for i in tqdm(estimator_range, desc="n_estimators search", unit="est"):
        clf = _make_clf(max_depth=6, n_estimators=i)
        clf.fit(X_train, le.transform(y_train))
        y_pred = clf.predict(X_test)
...
def find_best_learning_rate(X_train, X_test, y_train, y_test, le):
    ...
    for lr in tqdm(learning_rates, desc="learning_rate search", unit="lr"):
        clf = _make_clf(max_depth=6, n_estimators=100, learning_rate=lr)
        clf.fit(X_train, le.transform(y_train))
        y_pred = clf.predict(X_test)
...
def get_feature_importance(lgbm_model, X, le):
    sorted_idx = (-lgbm_model.feature_importances_).argsort()[:10]
    feature_data = pd.DataFrame(
        {
            "Feature": X.columns[sorted_idx],
            "Importance": lgbm_model.feature_importances_[sorted_idx],
        }
...
if __name__ == "__main__":
    ...
    # LightGBM requires numeric labels
    le = LabelEncoder()
    le.fit(y)
    ...
    # ── Final model ───────────────────────────────────────────────────────────
    print(f"\nTraining final LightGBM model  (max_depth=6, n_estimators={best_n}, lr={best_lr})...")
    lgbm_model = _make_clf(
        max_depth=6,
    ...
    lgbm_model.fit(
        ...
        callbacks=[
            log_evaluation(period=10),   # print loss every 10 rounds  ← final-model progress
        ],
    )
    ...
    feature_data = get_feature_importance(lgbm_model, X, le)
    ...
    with open("models/lgbm_results.pkl", "wb") as f:
        pickle.dump(results, f)

    print(f"\nLightGBM Accuracy: {results['accuracy']:.4f}")
    print("Saved → models/lgbm_results.pkl")