...
LGBM_GPU = dict(device="cpu", tree_method="auto", verbose=-1)


# ── LEWrapper ─────────────────────────────────────────────────────────────────
class _LEWrapper(BaseEstimator, ClassifierMixin):
    """Sklearn-compatible wrapper that encodes string labels before passing to XGBoost/LightGBM."""
...
def find_best_meta_learner(X_train, y_train, passthrough=False, cv=5):
    """
    ...
    Args:
        ...
        passthrough: If True, meta-learner gets original features + base predictions
        cv: Number of CV folds (default 5, increased from 3)
    """
    skf = StratifiedKFold(n_splits=cv, shuffle=True, random_state=42)
    meta_results = []

    print(f"  Running {cv}-fold CV for each meta-learner (passthrough={passthrough})...\n")
...
if __name__ == "__main__":
    ...
    print("\n[1/4] Evaluating base learners individually...")
    ...
    # ── Step 2: find best meta-learner WITHOUT passthrough ────────────────────
    print("\n[2/4] Searching best meta-learner (5-fold CV, passthrough=False)...")
    meta_results_no_passthrough = find_best_meta_learner(X_train, y_train, passthrough=False, cv=5)
    best_meta_no_pass = meta_results_no_passthrough.iloc[0]["meta_learner"]
    print(f"\n  → Best meta-learner (no passthrough): {best_meta_no_pass}")

    # ── Step 3: find best meta-learner WITH passthrough ───────────────────────
    print("\n[3/4] Searching best meta-learner (5-fold CV, passthrough=True)...")
    meta_results_passthrough = find_best_meta_learner(X_train, y_train, passthrough=True, cv=5)
    best_meta_pass = meta_results_passthrough.iloc[0]["meta_learner"]
    print(f"\n  → Best meta-learner (with passthrough): {best_meta_pass}")

    # ── Step 4: train final stacked model with best config ────────────────────
    # Choose the configuration with higher accuracy
    if meta_results_passthrough.iloc[0]["mean_cv_accuracy"] > meta_results_no_passthrough.iloc[0]["mean_cv_accuracy"]:
        best_meta_name = best_meta_pass
        use_passthrough = True
        print(f"\n[4/4] Training final model with passthrough=True, meta={best_meta_name}...")
    else:
        best_meta_name = best_meta_no_pass
        use_passthrough = False
        print(f"\n[4/4] Training final model with passthrough=False, meta={best_meta_name}...")
    ...
    stacked_row = pd.DataFrame([{
        "model":    f"stacking ({best_meta_name}, passthrough={use_passthrough})",
    ...
    results = {
        ...
        "accuracy":                   accuracy_score(y_test, y_pred),
        "conf_mat":                   conf_mat,
        "meta_results_no_passthrough": meta_results_no_passthrough,
        "meta_results_passthrough":    meta_results_passthrough,
    ...
    with open("models/stacking_improved_results.pkl", "wb") as f:
        pickle.dump(results, f)

    print(f"\nStacking Accuracy : {results['accuracy']:.4f}")
    print("\nModel Comparison:")
    print(comparison_df.to_string(index=False))
    print("\nMeta-learner comparison (no passthrough):")
    print(meta_results_no_passthrough.to_string(index=False))
    print("\nMeta-learner comparison (with passthrough):")
    print(meta_results_passthrough.to_string(index=False))
    print("\nSaved → models/stacking_improved_results.pkl")
